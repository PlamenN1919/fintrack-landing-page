...
// Irresistible Offer Section Functions
...
function initCountdownTimer() {
    // Set end time to 24 hours from now (or specific end time)
    ...
    function updateCountdown() {
        ...
        if (timeLeft > 0) {
            ...
            if (timeLeft < 3600000) { // Less than 1 hour
                const timerBlocks = document.querySelectorAll('.time-block');
                timerBlocks.forEach(block => {
                    block.style.animation = 'urgentTimer 0.5s ease-in-out infinite alternate';
                });
            }
        } else {
            // Timer expired - reset to 24 hours or show special message
            const expiredElements = document.querySelectorAll('.countdown-timer .time-value');
            expiredElements.forEach(el => el.textContent = '00');
            
            // Show expired message
            const warningEl = document.querySelector('.urgency-warning');
            if (warningEl) {
                warningEl.textContent = '🔥 Офертата изтече! Цената се удвоява всеки час!';
                warningEl.style.color = '#ff6b6b';
                warningEl.style.animation = 'expiredFlash 0.5s ease-in-out infinite';
            }
        }
    }
...
function initScarcityCounter() {
    let spotsLeft = 47; // Starting number
    
    function updateSpots() {
        const spotsEl = document.getElementById('spotsLeft');
        if (spotsEl && spotsLeft > 5) {
            // Randomly decrease spots every 3-8 minutes
            const randomDecrease = Math.floor(Math.random() * 3) + 1; // 1-3 spots
            spotsLeft = Math.max(5, spotsLeft - randomDecrease); // Never go below 5
            ...
            if (progressFill && progressText) {
                const percentage = ((500 - spotsLeft) / 500) * 100;
                progressFill.style.width = percentage + '%';
                progressText.textContent = `${500 - spotsLeft}/500 заети`;
            }
            
            // Add urgency effect when spots are low
            if (spotsLeft <= 20) {
                spotsEl.style.animation = 'urgentSpots 1s ease-in-out infinite';
                spotsEl.style.color = '#ff6b6b';
            }
            
            // Track scarcity event
            if (typeof gtag !== 'undefined') {
                gtag('event', 'scarcity_update', {
                    'spots_remaining': spotsLeft,
                    'urgency_level': spotsLeft <= 20 ? 'high' : 'medium'
                });
            }
        }
    }
    ...
    const updateInterval = Math.random() * 300000 + 180000; // 3-8 minutes in milliseconds
    ...
    document.addEventListener('visibilitychange', () => {
        if (!document.hidden && Math.random() > 0.7) { // 30% chance
            setTimeout(updateSpots, Math.random() * 5000 + 1000); // 1-6 seconds delay
        }
    });
}

// Offer animations and interactions
function initOfferAnimations() {
    // Animate prize icons on scroll
    const observerOptions = {
        threshold: 0.2,
        rootMargin: '0px'
    };
    ...
    const offerObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                // Animate total value
                const totalValue = entry.target.querySelector('.total-value');
                if (totalValue) {
                    setTimeout(() => {
                        totalValue.style.animation = 'valueReveal 1s ease-out';
                    }, 1000);
                }
                
                // Animate action container
                const actionContainer = entry.target.querySelector('.action-container');
                if (actionContainer) {
                    setTimeout(() => {
                        actionContainer.style.animation = 'ctaEntrance 1s ease-out';
                    }, 1200);
                }
            }
        });
    ...
    const offerSection = document.querySelector('.irresistible-offer-section');
    if (offerSection) {
        offerObserver.observe(offerSection);
        
        // Initially hide benefit items for animation
        const benefitItems = offerSection.querySelectorAll('.benefit-item');
    ...
    // Enhanced prize icon interactions
    const prizeIcons = document.querySelectorAll('.prize-icon');
    prizeIcons.forEach((icon, index) => {
        icon.addEventListener('mouseenter', () => {
            icon.style.transform = 'scale(1.3) rotate(10deg)';
            icon.style.boxShadow = '0 20px 50px rgba(255, 193, 7, 0.6)';
        });
        
        icon.addEventListener('mouseleave', () => {
            icon.style.transform = 'scale(1) rotate(0deg)';
            icon.style.boxShadow = '0 10px 30px rgba(255, 193, 7, 0.2)';
        });
        
        // Random pulse effect
        setInterval(() => {
            icon.style.animation = 'prizeFloat 8s ease-in-out infinite';
        }, Math.random() * 10000 + 5000);
    });
}

// Main offer claim function
function claimOffer() {
    // Track conversion attempt
    if (typeof gtag !== 'undefined') {
        gtag('event', 'offer_claim_attempt', {
            'event_category': 'conversion',
            'event_label': 'irresistible_offer',
            'spots_remaining': document.getElementById('spotsLeft')?.textContent || 'unknown'
        });
    }
    
    // Show loading state
    const ctaButtons = document.querySelectorAll('.mega-cta-btn, .urgency-btn');
    ctaButtons.forEach(btn => {
        const originalText = btn.innerHTML;
        btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> ОБРАБОТВА СЕ...';
        btn.disabled = true;
    ...
    const offerSection = document.querySelector('.irresistible-offer-section');
    if (offerSection) {
        offerSection.style.animation = 'offerClaimed 1s ease-out';
    }
}
...
function showOfferSuccess() {
    ...
    successOverlay.innerHTML = `
        ...
        ">
            <div style="
                background: linear-gradient(45deg, rgba(40, 167, 69, 0.9), rgba(255, 193, 7, 0.9));
                padding: 50px;
                border-radius: 25px;
                text-align: center;
                max-width: 500px;
            ...
            ">
                ...
                <h2 style="margin-bottom: 20px;">ПОЗДРАВЛЕНИЯ!</h2>
                <p style="margin-bottom: 30px;">Вашето безплатно място е запазено! Ще получите инструкции на вашия email в следващите минути.</p>
    ...
    if (spotsEl) {
        ...
        spotsEl.textContent = Math.max(1, currentSpots - 1);
    }
}

// Additional CSS animations for offer section
const offerStyles = `
@keyframes urgentTimer {
    ...
    to { 
        background: linear-gradient(45deg, rgba(255, 107, 107, 0.6), rgba(255, 193, 7, 0.6));
        transform: scale(1.05);
    }
}
...
// Inject offer styles
const offerStyleSheet = document.createElement('style');
offerStyleSheet.textContent = offerStyles;
document.head.appendChild(offerStyleSheet);
